...
class LSrouter(Router):
    ...
    def calPath(self):
        ...
        for addr, nbcost in self.routersLSP[self.addr].nbcost.items():
            Q.put((addr, nbcost, addr))
        self.info = ""   # 调试语句
        while not Q.empty():
            Addr, Cost, Next = Q.get(False)
            ...
            if Q.empty():
                self.info += str(D) + "\n"
                self.info += str(p) + "  "
                router = min(D, key=D.get)
                self.info += str(router) + "\n"
                self.routersCost[router] = D[router]
                self.routersNext[router] = p[router]
                S[router] = D[router]
 
                if router in self.routersLSP:
                    for addr,nbcost in self.routersLSP[router].nbcost.items():
                        self.info += addr +" "+ str(nbcost) + "**"
                        if addr not in S:  # if addr not in self.routersCost:
                            if (addr not in D) or (D[router] + nbcost < D[addr]):
                                D[addr] = D[router] + nbcost
                                p[addr] = p[router]
