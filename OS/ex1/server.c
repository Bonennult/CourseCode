...
void *SERVER(void* arg){
	...
	while(1){
		...
		printf("***************\ncustomer ID        : %d\nserver ID          : %d\nservice code       : %d\nentering time      : %d\nservice start time : %d\nleaving time       : %d\n***************\n\n", customer[cust_id].customer_id+1, serv_id, temp->serv_code, customer[cust_id].entering_time, customer[cust_id].serv_start_time, customer[cust_id].serv_start_time + customer[cust_id].serv_time);
		free(temp);
...
void main(){
	...
	for(i=0; i<count; i++){
		Sleep(customer[i].entering_time - customer[i-1].entering_time);
		pthread_create(&cust_thread_id[i], NULL, CUSTOMER, (void*)i);
	}
