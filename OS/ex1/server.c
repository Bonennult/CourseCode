...
void *SERVER(void* arg){
	...
	while(1){
		...
		Sleep((customer[cust_id].serv_time)*1000);
		printf("***************\n
			customer ID        : %d\n
			server ID          : %d\n
			service code       : %d\n
...
void *CUSTOMER(void* arg){
	...
	tail->customer_id = temp1.customer_id;
...
void main(){
	...
	while(fscanf(fp, "%d %d %d", &id, &time1, &time2) != -1){
		customer[i].customer_id = id;
		customer[i].entering_time = time1;
		customer[i].serv_time = time2;
		i++;
	}
	fclose(fp);
	...
	for(i=0; i<CUST_NUM; I++)
		pthread_create(&cust_thread_id[i], NULL, CUSTOMER, (void *)customer[i]);
	for(i=0; i<CUST_NUM; i++)                     //wait for all customer threads end
